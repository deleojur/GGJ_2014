...
public class EnemieMove : MonoBehaviour
{
		...
		/*
		IEnumerator UpdateDirectionTowardsPlayer ()
		{
				while (true) {

						//TODO: find out if this is necesarry
						if (slowingDown || startingUp)
								yield return null;

						//Get target's transform from given name
						Transform target = GameObject.Find (nameTargetObject).transform;	

						//Check if direction needs to be switched
						facingLeft = (transform.position.x > target.position.x) ? true : false;
						yield return new WaitForSeconds (checkTargetDelay);
				}
		}*/
